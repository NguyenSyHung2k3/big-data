...
df = scaler.fit(df).transform(df)
reference_row = df.filter(col("Track ID") == "7EhJMeBeptBfJh8kkjZXjw").select("scaledFeatures").first()
...
if reference_row is None:
    ...
    reference_vector_broadcast = spark.sparkContext.broadcast(reference_vector)
    # Define the UDF for Euclidean distance
    @udf("double")
    def compute_distance(vec):
        ref_vec = reference_vector_broadcast.value
        return float(np.linalg.norm(np.array(vec.toArray()) - np.array(ref_vec)))

    df = df.withColumn("distance", compute_distance(col("scaledFeatures")))

        # Add the distance column
    recommendations = df.orderBy("distance").limit(10)
    recommendations.select("Track ID", "Track Name", "distance").show(truncate=False)